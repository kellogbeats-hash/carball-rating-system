# 🚗 MTA CarBall Rating System

<div align="center">

[![English](https://img.shields.io/badge/🇬🇧_English-README-blue?style=for-the-badge)](docs/en/README.md)
[![Türkçe](https://img.shields.io/badge/🇹🇷_Türkçe-README-red?style=for-the-badge)](docs/tr/README.md)

<br>

![GitHub stars](https://img.shields.io/github/stars/pykellog/carball-rating-system?style=social)
![GitHub forks](https://img.shields.io/github/forks/pykellog/carball-rating-system?style=social)
![GitHub watchers](https://img.shields.io/github/watchers/pykellog/carball-rating-system?style=social)

</div>

---

## 📊 View Player Rankings / Oyuncu Sıralamasını Görüntüle

<div align="center">

### 🌐 **[View Player Rankings](https://pykellog.github.io/carball-rating-system/)** 📊

[![View Rankings](https://img.shields.io/badge/📊_View_Rankings-Click_Here-3498db?style=for-the-badge&logo=githubpages)](https://pykellog.github.io/carball-rating-system/)

**🔍 Search any player, sort by stats, and explore full CarBall rankings!**

> 📌 **Note:** Rankings are updated manually when new data is available.  
> 📅 **Last Update:** Check the top of the page for the latest update date.

</div>

---

## 📊 System Overview / Sistem Özeti

<div align="center">

| 🇬🇧 English | 🇹🇷 Türkçe |
|:---:|:---:|
| **FINAL RATING = RAW SKILL × EXPERIENCE MULTIPLIER** | **SON DERECELENDİRME = HAM YETENEK × DENEYİM ÇARPANI** |

</div>

### What This System Does / Bu Sistem Ne Yapar?

- ✅ **468+ Players** ranked with detailed stats
- ✅ **Search** by nickname
- ✅ **Sortable columns** for all statistics
- ✅ **Tier system** from S-Tier to E-Tier
- ✅ **Role classification** (Goalkeeper, Striker, Playmaker)
- ✅ **Full transparency** - all formulas are public

---

## 📅 Update Schedule / Güncelleme Periyodu

<div align="center">

| 🇬🇧 English | 🇹🇷 Türkçe |
|:---:|:---:|
| Rankings are updated **manually** when new data is exported from the server. | Sıralamalar, sunucudan yeni veriler alındıkça **manuel olarak** güncellenir. |
| Check the **"Last Updated"** field on the site for the most recent update date. | En son güncelleme tarihi için sitedeki **"Son Güncelleme"** alanını kontrol edin. |

</div>

---

## 🎯 Features / Özellikler

| Feature | Description |
|---------|-------------|
| 📊 **Player Rankings** | View all players sorted by rating |
| 🔍 **Search** | Find any player by nickname instantly |
| 📈 **Stats** | Goals, Assists, Saves, Win Rate, MVP and more |
| 🏷️ **Tiers** | Color-coded tiers from S (best) to E (new) |
| 🎭 **Roles** | Tactical role classification for each player |
| 📱 **Mobile Friendly** | Works perfectly on all devices |
| 📅 **Update Info** | Shows when rankings were last updated |

---

## 📈 Statistics / İstatistikler

<div align="center">

| Metric | Value |
|:---:|:---:|
| 👥 **Total Players** | 468 |
| 🏆 **Top Rating** | 10.0 |
| ⭐ **S-Tier Players** | 13 |
| 📊 **Total Matches** | 3.5M+ |

</div>

---

## 🔧 Quick Links / Hızlı Bağlantılar

<div align="center">

| 📖 Documentation | 🌐 Player Rankings |
|:---:|:---:|
| [![English Docs](https://img.shields.io/badge/🇬🇧_English_Docs-2c3e50?style=for-the-badge)](docs/en/README.md) | [![View Rankings](https://img.shields.io/badge/📊_View_Rankings-3498db?style=for-the-badge)](https://pykellog.github.io/carball-rating-system/) |
| [![Türkçe Doküman](https://img.shields.io/badge/🇹🇷_Türkçe_Doküman-2c3e50?style=for-the-badge)](docs/tr/README.md) | [![Short Link](https://img.shields.io/badge/🔗_Short_Link-2ecc71?style=for-the-badge)](https://tinyurl.com/ffs-carball) |

</div>

---

## 🛠️ Tech Stack / Teknoloji

- **Frontend:** HTML, CSS, JavaScript
- **Table:** DataTables.js
- **Hosting:** GitHub Pages
- **Analytics:** GoatCounter
- **Data Format:** CSV (manually updated)

---

## 📝 License / Lisans

<div align="center">

This project is licensed under the **MIT License**.
See the [LICENSE](LICENSE) file for details.

Bu proje **MIT Lisansı** ile lisanslanmıştır.
Detaylar için [LICENSE](LICENSE) dosyasına bakın.

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

</div>

---

## 👨‍💻 Developer / Geliştirici

<div align="center">

**Developed by [keLLog](https://github.com/pykellog)**

[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/pykellog)

</div>

---

## 📅 Last Updated / Son Güncelleme

**August 2026**

---

<div align="center">

### ⭐ Star this repo if you find it useful!

[![Star](https://img.shields.io/badge/⭐_Star_this_repo-ffd700?style=for-the-badge)](https://github.com/pykellog/carball-rating-system)

</div>
